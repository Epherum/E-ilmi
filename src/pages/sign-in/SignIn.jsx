...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (signInWithEmail) {
      ...
      let url = isSellerAccount
        ? "https://localhost:8000/api/authentification/seller/login"
        : "https://localhost:8000/api/authentification/client/login";
      if (isSellerAccount) {
      ...
      let url = isSellerAccount
        ? "https://localhost:8000/api/authentification/seller/login-with-phone-number"
        : "https://localhost:8000/api/authentification/client/login-with-phone-number";
      if (isSellerAccount) {
        let response = await fetch(url, {
          method: "POST",
          body: JSON.stringify(request),
  ...
  return (
    <div className={classes.sing_in_page}>
      <div className={classes.sign_up_page_container} data-aos="zoom-in">
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <h5 className={classes.user_type_choice_title}>
                  Avec quel type de compte souhaitez-vous vous connecter ?
                </h5>
              </Grid>
              ...
              <Grid item xs={12} sx={{ my: 1 }}>
                <FormControl fullWidth variant="outlined">
                  ...
                  <OutlinedInput
                    color={
                      (signInWithEmail && !emailIsCorrect) ||
                      (!signInWithEmail && !phoneNumberIsCorrect)
                        ? "error"
                        : "primary"
                    }
                    fullWidth
              ...
              <Grid item xs={12}>
                ...
                <Alert
                  severity="error"
                  sx={!passwordIsCorrect ? { mt: 0.5 } : { display: "none" }}
                >
              ...
              <Grid item xs={12}>
                <FormGroup>
                  <FormControlLabel
                    disableTypography
                    sx={{ fontFamily: "'Roboto Mon', monospace" }}
                    control={
                      <Checkbox
                        checked={remenberMe}
                        onChange={handleChangeRemenberMe}
                        size="small"
                      />
                    }
              ...
              <Grid item xs={12}>
                ...
                <div className={classes.link_to_sign_up_wrapper}>
                  ...
                  <Link className={classes.link_to_sign_up} to="/sign-up">
                    Créez un compte
                  </Link>
                </div>
              </Grid>
        ...
        <div className={classes.sign_up_page_container_footer}></div>
      </div>
    </div>
  );
};
